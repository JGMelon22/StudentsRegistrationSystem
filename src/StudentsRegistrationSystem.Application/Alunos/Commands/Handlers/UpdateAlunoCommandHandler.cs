...
public class UpdateAlunoCommandHandler : IRequestHandler<UpdateAlunoCommand, Result<AlunoResponse>>
{
    ...
    public UpdateAlunoCommandHandler(IAlunoRepository alunoRepository, AppDbContext context,
        ILogger<UpdateAlunoCommandHandler> logger)
    {
    ...
    public async Task<Result<AlunoResponse>> Handle(UpdateAlunoCommand command, CancellationToken cancellationToken)
    {
        ...
        if (emailExists)
        {
            _logger.LogWarning("Tentativa de atualizar aluno com e-mail já existente. Email: {Email}",
