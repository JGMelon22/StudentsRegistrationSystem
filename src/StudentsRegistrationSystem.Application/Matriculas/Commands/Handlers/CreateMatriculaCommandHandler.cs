...
public class CreateMatriculaCommandHandler : IRequestHandler<CreateMatriculaCommand, Result<MatriculaResponse>>
{
    ...
    public async Task<Result<MatriculaResponse>> Handle(CreateMatriculaCommand command,
        CancellationToken cancellationToken)
    {
        ...
        if (jaMatriculado)
        {
            _logger.LogWarning("Aluno já matriculado. AlunoId: {AlunoId}, CursoId: {CursoId}", command.Request.AlunoId,
                command.Request.CursoId);
