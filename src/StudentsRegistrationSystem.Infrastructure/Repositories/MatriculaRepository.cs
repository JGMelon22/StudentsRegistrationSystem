...
public class MatriculaRepository : Repository<Matricula>, IMatriculaRepository
{
    ...
    public async Task<Matricula?> GetMatriculaAtivaAsync(Guid alunoId, Guid cursoId,
        CancellationToken cancellationToken = default)
    {
        return await _dbSet
            .Include(m => m.Aluno)
    ...
    {
        return await _dbSet.AnyAsync(m => m.AlunoId == alunoId && m.Ativa);
    }

    public async Task<IEnumerable<Matricula>> ObterMatriculasDesativadasPorAlunoId(Guid alunoId,
        CancellationToken cancellationToken = default)
    {
